...
from MetaPath import get_features_tag
import numpy as np
import tqdm
import os
import random
...
class EmotionRecognizer:
    ...
    def __init__(self, model=None, **kwargs):
        ...
        self.audio_config = get_features_tag(self.features)
        # datasets
    ...
    def best_estimators(self):
        ...
        return best_estimators(self.classification)
    ...
    def write_csv(self):
        ...
        for train_csv_file, test_csv_file in zip(self.train_desc_files, self.test_desc_files):
            ...
            if self.emodb_name in train_csv_file:
                create_emodb_csv(self.emotions, train_name=train_csv_file, test_name=test_csv_file, verbose=self.verbose)
                if self.verbose:
                    print("[+] Generated EMO-DB CSV File")
            elif self.tess_ravdess_name in train_csv_file:
                create_ravdess_csv(self.emotions, train_name=train_csv_file, test_name=test_csv_file, verbose=self.verbose)
                if self.verbose:
    ...
    def load_data(self):
        ...
        if not self.data_loaded:
            result = load_data_from_meta(self.train_desc_files, self.test_desc_files, self.audio_config, self.classification,
