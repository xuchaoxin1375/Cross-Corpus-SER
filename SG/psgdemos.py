...
def find_in_file(
    ...
    for file in demo_files_dict:
        try:
            ...
            with open(full_filename, "rb", 0) as f, mmap.mmap(
                ...
                if regex:
                    ...
                    window.refresh()
                    matches = re.finditer(
                        bytes("^.*(" + string + ").*$", "utf-8"), s, re.MULTILINE
                    )
                    if matches:
                    ...
                    if matches:
                        if show_first_match:
                            ...
                            if (
                                not all(x in matched_str for x in ("b'", "="))
                                and len(matched_str) < 500
                            ):
        ...
        for key in matched_dict:
            ...
            try:
                ...
                for _match in matched_dict[key]:
                    ...
                    if verbose:
                        sg.cprint(
                            f"Line: {line_num_match} ", c="white on purple", end=""
                        )
...
def settings_window():
    ...
    layout = [
        ...
        [
            sg.In(
                sg.user_settings_get_entry("-editor program-", ""), k="-EDITOR PROGRAM-"
            ),
            sg.FileBrowse(),
        ],
        ...
        [
            sg.In(
                sg.user_settings_get_entry("-explorer program-"), k="-EXPLORER PROGRAM-"
            ),
            sg.FileBrowse(),
        ],
    ...
    left_col = sg.Column(
        [
            ...
            [
                ...
                sg.Input(
                    size=(25, 1), enable_events=True, key="-FIND-", tooltip=find_tooltip
                ),
    ...
    choose_folder_at_top = sg.pin(
        sg.Column(
            [
                [
                    sg.T(
                        "Click settings to set top of your tree or choose a previously chosen folder"
                    ),
                    sg.Combo(
                        sorted(sg.user_settings_get_entry("-folder names-", [])),
...
def main():
    ...
    try:
        ...
        if major_version < 4 or (major_version == 4 and minor_version < 32):
            sg.popup(
                "Warning - Your PySimpleGUI version is less then 4.35.0",
